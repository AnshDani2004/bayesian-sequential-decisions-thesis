...
The Risk-Constrained agent achieves \textbf{100\% survival} (95\% CI: [96.4\%, 100\%], $n=100$ Monte Carlo runs) and \textbf{0\% drawdown breach} across all tail parameters, validating the CPPI mechanism.

\begin{remark}[Discrete-Time Caveat]
These results are probabilistic robustness statements, not mathematical guarantees. In discrete time with jump risk, floor breach remains possible under extreme market moves. The 100\% empirical survival reflects the finite-sample regime and chosen parameters --- see Theorem~\ref{thm:impossibility} for asymptotic limitations.
\end{remark}
...
\begin{enumerate}
    ...
    \item \textbf{Survival Success}: Risk-Constrained agent achieves 100\% empirical survival rate (95\% CI: [96.4\%, 100\%]) with 0\% drawdown breach across $n=100$ trials
